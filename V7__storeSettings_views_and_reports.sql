...
-- ================================================================================
-- 2. VIEW: ar_balance_per_customer (Công nợ phải thu từng khách hàng)
-- ================================================================================
-- Tính công nợ khách hàng (AR - Accounts Receivable)
-- Công thức: balance = tổng đơn hàng - tổng đã trả - điều chỉnh giảm + điều chỉnh tăng
-- CTE:
--   ord: tổng giá trị đơn hàng (trừ đơn cancelled)
--   pay: tổng số tiền khách đã trả
--   adj: tổng điều chỉnh (positive = giảm nợ, negative = tăng nợ)
...
adj as (
  select ca.buyer_id,
         sum(case when ca.amount > 0 then ca.amount else 0 end) as adj_positive,  -- Tổng điều chỉnh giảm nợ
         sum(case when ca.amount < 0 then -ca.amount else 0 end) as adj_negative  -- Tổng điều chỉnh tăng nợ
...
select
  p.id as buyer_id,
  p.name,
  coalesce(sum(ord.net_total),0)        -- Tổng đơn hàng
  - coalesce(sum(pay.paid_total),0)     -- Trừ tổng đã trả
  - coalesce(max(adj.adj_positive),0)   -- Trừ điều chỉnh giảm nợ
  + coalesce(max(adj.adj_negative),0)   -- Cộng điều chỉnh tăng nợ
  as balance                            -- Số tiền còn nợ (> 0 = khách nợ, < 0 = dư trả)
from profiles p
...
create table if not exists store_settings (
  ...
  store_name text not null,                                                  -- Tên cửa hàng (hiển thị)
  legal_name text,                                                           -- Tên pháp lý (trên giấy phép)
  ...
  locale text not null default 'vi-VN',                                      -- Ngôn ngữ/vùng miền
...
select
  ...
  pu.size_label,                                                         -- Quy cách (VD: "50kg", "1L")
...
-- ================================================================================
-- 9. VIEW: gr_print_header (Header phiếu nhập kho từ NCC - để in)
-- ================================================================================
-- Lấy thông tin header của phiếu nhập kho từ nhà cung cấp
-- Bao gồm: thông tin nhập kho, NCC, kho, và cửa hàng
-- Dùng để in phiếu nhập kho khi nhận hàng
create or replace view gr_print_header as
with ss as (select * from store_settings order by id desc limit 1)
select
  gr.id as receipt_id,                       -- ID phiếu nhập
  gr.received_at as received_at,             -- Ngày nhập kho
  s.name as supplier_name,                   -- Tên nhà cung cấp
  s.phone as supplier_phone,                 -- SĐT NCC
  s.address as supplier_address,             -- Địa chỉ NCC
  w.name as warehouse_name,                  -- Tên kho nhập vào
  ss.store_name,                             -- Tên cửa hàng
  ss.address as store_address,               -- Địa chỉ cửa hàng
  ss.phone as store_phone,                   -- SĐT cửa hàng
  ss.email as store_email                    -- Email cửa hàng
...
-- ================================================================================
-- 10. VIEW: gr_print_lines (Chi tiết phiếu nhập kho từ NCC - để in)
-- ================================================================================
-- Lấy các dòng sản phẩm nhập kho từ NCC
-- Bao gồm: STT, SKU, tên SP, quy cách, số lượng (không có giá)
create or replace view gr_print_lines as
select
  gri.receipt_id,                                                        -- ID phiếu nhập
  row_number() over (partition by gri.receipt_id order by gri.id) as line_no, -- STT dòng
  pu.sku,                                                                -- Mã SKU
  pr.name as product_name,                                               -- Tên sản phẩm
  pu.size_label,                                                         -- Quy cách
  gri.quantity                                                           -- Số lượng nhập
